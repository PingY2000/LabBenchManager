# LabBenchManager - 实验室台架管理系统

`LabBenchManager` 是一个基于 Web 的内部应用程序，旨在简化和自动化实验室测试台架（Lab Bench）的分配、调度、和管理流程。它为测试工程师、请求者和管理员提供了一个集中的平台来查看台架状态、提交测试请求、管理测试计划和审批测试报告。
